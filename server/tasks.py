...
async def refresh_prices_once() -> dict:
    """手动刷新价格一次并返回结果"""
    from database import get_db_connection
    from price_fetcher import get_price_from_market, price_fetch_logging, _price_cache_clear
    ...
    try:
        ...
        for row in unique_positions:
            ...
            # 清除缓存以强制刷新
            _price_cache_clear(symbol, market, token_id if token_id else None)
        ...
        for item in results:
            if item["price"] is not None:
                ...
                _record_price_failure(item["key"], now_ts)
        ...
        print(
            f"[Manual Refresh] Complete: total={total_count}, updated={success_count}, failed={failed_count}"
        )
...
async def cache_warmup_loop():
    """Background task to periodically warm up price cache for popular symbols."""
    from price_fetcher import _warmup_cache, _CACHE_WARMUP_ENABLED, _CACHE_WARMUP_SYMBOLS

    if not _CACHE_WARMUP_ENABLED:
        print("[Cache Warmup] Disabled by configuration")
        return

    warmup_interval = _env_int("CACHE_WARMUP_INTERVAL", 300, minimum=60)
    initial_delay = _env_int("CACHE_WARMUP_INITIAL_DELAY", 10, minimum=0)

    await asyncio.sleep(initial_delay)

    print(
        f"[Cache Warmup] Started with interval={warmup_interval}s, "
        f"symbols={_CACHE_WARMUP_SYMBOLS}"
    )

    while True:
        try:
            result = await asyncio.to_thread(_warmup_cache, _CACHE_WARMUP_SYMBOLS)
            print(
                f"[Cache Warmup] Complete: total={result['total']}, "
                f"success={result['success']}, failed={result['failed']}"
            )
        except Exception as e:
            print(f"[Cache Warmup Error] {e}")

        print(f"[Cache Warmup] Next warmup in {warmup_interval} seconds")
...
BACKGROUND_TASK_REGISTRY = {
    ...
    "cache_warmup": cache_warmup_loop,
}
